...
func (r *OpenStackNetAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	defer func(cond *ospdirectorshared.Condition) {
		...
		instance.Status.Conditions.UpdateCurrentCondition(
			cond.Type,
			ospdirectorshared.ConditionReason(cond.Message),
			cond.Message,
		)
	...
	if instance.Spec.AttachConfiguration.NodeSriovConfigurationPolicy.DesiredState.Port != "" {
		...
		if err := r.ensureSriov(ctx, instance); err != nil {
			...
			cond.Type = ospdirectorshared.ConditionType(ospdirectorv1beta1.NetAttachError)
			return ctrl.Result{}, err
		}
		...
		//
		if k8s_errors.IsNotFound(err) ||
			cond.Reason == ospdirectorshared.ConditionReason(nmstateshared.NodeNetworkConfigurationPolicyConditionSuccessfullyConfigured) {
			//
...
func (r *OpenStackNetAttachmentReconciler) createOrUpdateNodeNetworkConfigurationPolicy(
	...
	if op != controllerutil.OperationResultNone {
		...
		cond.Type = ospdirectorshared.ConditionType(ospdirectorv1beta1.NetAttachConfiguring)
...
func (r *OpenStackNetAttachmentReconciler) getNodeNetworkConfigurationPolicyStatus(
	...
	if err != nil {
		...
		cond.Reason = ospdirectorshared.ConditionReason(ospdirectorshared.CommonCondReasonNNCPError)
		cond.Type = ospdirectorshared.ConditionType(ospdirectorv1beta1.NetAttachError)
		err = common.WrapErrorForObject(cond.Message, instance, err)
		return err
	}
	...
	if networkConfigurationPolicy.Status.Conditions != nil && len(networkConfigurationPolicy.Status.Conditions) > 0 {
		...
		if condition != nil {
			...
			cond.Reason = ospdirectorshared.ConditionReason(condition.Reason)
			cond.Type = ospdirectorshared.ConditionType(condition.Type)
			...
			if condition.Type == nmstateshared.NodeNetworkConfigurationPolicyConditionAvailable &&
				...
				cond.Type = ospdirectorshared.ConditionType(ospdirectorv1beta1.NetAttachConfigured)
...
func (r *OpenStackNetAttachmentReconciler) cleanupNodeNetworkConfigurationPolicy(
	...
	if err != nil &&
		cond.Reason != ospdirectorshared.ConditionReason(nmstateshared.NodeNetworkConfigurationPolicyConditionFailedToConfigure) {
		return ctrl.Result{}, err
	}
	...
	if err != nil {
		...
		cond.Reason = ospdirectorshared.ConditionReason(cond.Message)
		cond.Type = ospdirectorshared.ConditionType(ospdirectorv1beta1.NetAttachError)
